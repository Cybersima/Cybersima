...
This tree is **SecureTrade 2.1.0**, the packaged product. Earlier scanner work lives on `cursor/pulsearb-multimarket-scanner-c11f` (v0.1.0).
...
| **v2.0.0 SecureTrade 2** | Packaged product | Installable command center + Docker engine, Guardian™, Trust Score, Learn/Assist/Auto, Academy, decision journal, capital protection, scam defense, takeover detection, and the recovered v0.3.8b pipeline implemented on the v0.1 scanner foundation. |
| **v2.1.0 SecureTrade 2** | This product | Forex Desk: 3-second Yahoo tape, 30s/1m/2m/3m/4m/5m/1h/1d pattern recognition, BUY/SELL entries, stop/target/time/reversal exits, FX triangle dislocations, plus the v2.0 Guardian pipeline. |
