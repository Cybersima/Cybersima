...
from securetrade.engine.broker import PaperBroker
from securetrade.engine.risk import RiskManager
from securetrade.models import KillSource, Leg, Opportunity, OpportunityKind
...
@pytest.mark.asyncio
async def test_paper_submit_then_realize() -> None:
...
def test_daily_loss_auto_kills() -> None:
    risk = RiskManager(daily_loss_limit_usdt=10, cooldown_seconds=0)
    risk.record_pnl(-10)
    decision = risk.allow(5)
    assert not decision.allowed
    assert risk.killed
    assert risk.kill_source == KillSource.RISK.value
